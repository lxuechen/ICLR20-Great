class Vocabulary():
    ...
    def lookup(self, token):
        return self.w2i[token] if token in self.w2i else self.w2i[
            "<PAD>"]  # Ignore truly unknown tokens; only happens when specific characters were never seen in
        # training data.
        ...
        while ix < len(token):
            if ix == len(token) - 2:
                ...
                if not candidates:
                    ...
                    # Only sub-tokens that match the next characters and don't leave the end-of-word marker left by
                    # itself
