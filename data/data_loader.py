...
class DataLoader():
    ...
    def batcher(self, mode="train"):
        ...
        ds = tf.data.Dataset.from_generator(lambda mode: self.to_batch(dataset, mode), output_types=(
            tf.dtypes.int32, tf.dtypes.int32, tf.dtypes.int32, tf.dtypes.int32, tf.dtypes.int32), args=(mode,))
        ds = ds.prefetch(1)
        return ds
    ...
    def to_batch(self, sample_generator, mode):
        ...
        def make_batch(buffer):
            ...
            # Targets and candidates both have an added batch dimension as well, and are otherwise just a list of
            # indices
            target_batches = tf.repeat(tf.range(batch_dim), [len(targets) for targets in batch[3]])
            repair_targets = tf.cast(tf.concat(batch[3], axis=0),
                                     'int32')  # Explicitly cast to int32 since repair targets can be empty,
            # in which case TF defaults to float32
