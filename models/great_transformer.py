...
class AttentionLayer(tf.keras.layers.Layer):
    """Implementation of multi-headed attention with optional edge-bias.
        ...
        This class supports self-attention and key-value attention, with (non-optional) masks. If bias_dim is not
        None, the attention computation(s) assumes that a (sparse) bias vector is provided, formatted like: (
        edge_type, batch_index, key_index, query_index). Bias edge types are embedded in the same dimension as each
        head's attention and projected to a scalar before being inserted into the attention computation as (q + b) * k.
    """
    ...
    def call(self, states, key_states, masks, attention_bias):
        # Compute key, query and value vectors, reshaped to [Batch, Heads, Time, Dim] where Dim is
...
class Transformer(tf.keras.layers.Layer):
    ...
    def call(self, states, masks, attention_bias, training):
        real_dropout_rate = self.dropout_rate * tf.cast(training,
                                                        'float32')  # Easier for distributed training than an
    ...
    # NOTE: tentatively does not support attention bias within the query itself; extending this should be
    # straightforward.
    @tf.function(input_signature=[tf.TensorSpec(shape=(None, None, None), dtype=tf.float32),
                                  tf.TensorSpec(shape=(None, None, None, None), dtype=tf.float32),
                                  tf.TensorSpec(shape=(None, None, None), dtype=tf.float32),
