...
class VarMisuseModel(tf.keras.layers.Layer):
    ...
    def build(self, _):
        ...
        for kind in desc:
            ...
            elif kind == 'transformer':  # Same as above, but explicitly without bias_dim set -- defaults to regular
                # Transformer.
    ...
    def call(self, tokens, token_mask, edges, training):
        ...
        # Track whether any position-aware model processes the states first. If not, add positional encoding to
        # ensure that e.g. GREAT and GGNN
        ...
        # which is hard to predict for e.g. Transformers and GGNNs without either sequential awareness or a special
        ...
        for model in self.stack:
            if isinstance(model, rnn.RNN):  # RNNs simply use the states
                ...
                states = model(states, edges, training=training)
                            ...
                            great_transformer.Transformer):  # For Transformers, reverse edge directions to match
        ...
        predictions = tf.transpose(self.prediction(states),
                                   [0, 2, 1])  # Convert to [batch, 2, seq-length] for convenience.
        return predictions

    @tf.function(input_signature=[tf.TensorSpec(shape=(None, 2, None), dtype=tf.float32),
                                  tf.TensorSpec(shape=(None, None), dtype=tf.int32),
    ...
    def get_loss(self, predictions, token_mask, error_locations, repair_targets, repair_candidates):
        ...
        is_buggy = tf.cast(tf.clip_by_value(error_locations, 0, 1),
                           'float32')  # 0 is the default position for non-buggy samples
        ...
        # Store two metrics: the accuracy at predicting specifically the non-buggy samples correctly (to measure
        # false alarm rate), and the accuracy at detecting the real bugs.
        no_bug_pred_acc = tf.reduce_sum((1 - is_buggy) * loc_accs) / (
                1e-9 + tf.reduce_sum(1 - is_buggy))  # Take mean only on sequences without errors
        ...
        candidate_mask = tf.scatter_nd(repair_candidates, tf.ones(tf.shape(repair_candidates)[0]),
                                       tf.shape(pointer_logits))
        pointer_logits += (1.0 - candidate_mask) * tf.float32.min
        pointer_probs = tf.nn.softmax(pointer_logits)

        # Aggregate probabilities at repair targets to get the sum total probability assigned to the correct variable
        # name
        ...
        target_loss = tf.reduce_sum(is_buggy * -tf.math.log(target_probs + 1e-9)) / (
                1e-9 + tf.reduce_sum(is_buggy))  # Only on errors

        # To simplify the comparison, accuracy is computed as achieving >= 50% probability for the top guess
        # (as opposed to the slightly more accurate, but hard to compute quickly, greatest probability among distinct
        # variable names).
